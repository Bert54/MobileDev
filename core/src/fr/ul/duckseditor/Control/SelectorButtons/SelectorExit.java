...
public class SelectorExit extends SelectorButton {

    protected float size = Math.max(CAMERAWIDTH/40f, CAMERAHEIGHT/40f) * 3;

    protected static float positionX = CAMERAWIDTH/2.7f;
    protected static float positionY = CAMERAHEIGHT/8f;

    public SelectorExit(World world) {
        super();
    ...
    public void draw(SpriteBatch sb) {
        sb.draw(TextureFactory.getInstance().getCancel(), positionX, positionY, this.size, this.size);
    }
}
