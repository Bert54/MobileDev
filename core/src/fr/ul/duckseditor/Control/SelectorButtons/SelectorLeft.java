...
public class SelectorLeft extends SelectorButton {

    protected float size = Math.max(CAMERAWIDTH/40f, CAMERAHEIGHT/40f) * 4;

    protected static float positionX = CAMERAWIDTH/16f;
    protected static float positionY = CAMERAHEIGHT/2f;

    public SelectorLeft(World world) {
        super();
    ...
    public void draw(SpriteBatch sb) {
        sb.draw(TextureFactory.getInstance().getLeftArrow(), positionX, positionY, this.size, this.size);
    }
}
